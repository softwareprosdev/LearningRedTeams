...
describe('LabsService', () => {
  ...
  beforeEach(async () => {
    prisma = {
      ...
      labSession: { findFirst: jest.fn(), create: jest.fn(), update: jest.fn(), findMany: jest.fn(), findUnique: jest.fn() },
  ...
  it('completeLab throws when no active session', async () => {
    prisma.labSession.findFirst.mockResolvedValue(null);
    await expect(service.completeLab('lab-1', 'u1', ['o1'])).rejects.toThrow('No active lab session found');
  });

  it('completeLab updates session, awards progress and gamification when all objectives done', async () => {
    const lab = { id: 'lab-3', lessonId: 'lesson-1', objectives: [{ id: 'o1', points: 1 }, { id: 'o2', points: 2 }] };
    const session = { id: 's3', lab, status: 'RUNNING', lab: lab };

    prisma.labSession.findFirst.mockResolvedValue(session);
    prisma.labSession.update.mockResolvedValue({ id: 's3', status: 'STOPPED', completedObjectives: ['o1', 'o2'], score: 3, lab });
    prisma.progress.upsert.mockResolvedValue({});

    const res = await service.completeLab('lab-3', 'u3', ['o1', 'o2']);

    expect(prisma.labSession.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 's3' } }));
    expect(prisma.progress.upsert).toHaveBeenCalled();
    expect(gamification.awardPointsForEvent).toHaveBeenCalledWith('u3', PointEventType.LAB_COMPLETE, session.lab.lessonId);
